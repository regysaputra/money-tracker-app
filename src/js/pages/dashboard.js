...
const Dashboard = {
  ...
  async _initialData () {
    try {
      ...
      // console.log('user Transaction History : ', this._userTransactionsHistory)
  ...
  _populateTransactionsDataToCard (transactionsHistory = null) {
    ...
    if (!Array.isArray(transactionsHistory)) {
      throw new Error('Parameter transactionsHistory should be an array. The value is ${transactionsHistory}');
    }
  ...
  _populateDetailTransactionToModal (transactionRecord) {
    ...
    console.log('transactionRecord.date : ', transactionRecord.date)
    ...
    dateDetailRecord.textContent = new Date(transactionRecord.date.seconds * 1000).toString().slice(0, 24);
